...
public class Lab2 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < myPoint.length; i++) {
            ...
            System.out.print("Координата x для " + (i+1) + " точки: ");
            myPoint[i].setX(in.nextDouble());
            System.out.print("Координата y для " + (i+1) + " точки: ");
            myPoint[i].setY(in.nextDouble());
            System.out.print("Координата z для " + (i+1) + " точки: ");
            myPoint[i].setZ(in.nextDouble());

        }

        //Цикл для вывода данных о точках, которые мы вводили
        for (int i = 0; i < myPoint.length; i++) {
            System.out.println("Вывод координат " + (i+1) + " точки: (" + myPoint[i].getX() + " " + myPoint[i].getY() + " " + myPoint[i].getZ() + ")");
        }

        //Проверка на равность точек
        if (equals(myPoint[0], myPoint[1], myPoint[2])) {
    ...
    public static double computeArea (Point3d p1, Point3d p2, Point3d p3) //Вычисление площади треугольника
    {
        ...
        public static boolean equals(Point3d p1, Point3d p2, Point3d p3)
        {
